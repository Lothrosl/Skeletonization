...
class RoadThinningDataset(Dataset):
    def __init__(self, data_dir, image_size=(256, 256), transform=None, split='train', test_size=0.2, random_state=42):
        ...
        # Get all image files
        subset_size = 500 # or any smaller number you want

        self.image_files = sorted([f for f in os.listdir(data_dir) if f.startswith('image_') and f.endswith('.png')])[:subset_size]
        self.target_files = sorted([f for f in os.listdir(data_dir) if f.startswith('target_') and f.endswith('.png')])[:subset_size]
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, stride=2, padding =1, kernel_size=1),
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'ablation_model7.pth')
    ...
    plt.savefig('loss_curves_7.png')
    plt.close()
    
    # Load best model and evaluate
    model.load_state_dict(torch.load('ablation_model7.pth'))
